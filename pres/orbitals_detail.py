...
FRIENDSHIP_INITIATE_PROB = 0.2 # probability of friendship initation attempt
...
class Render(object):
  ...
  def render_edges_and_nodes(self):
    ...
    for i,j in zip(indsx[mask],indsy[mask]):
      ...
      self.ctx.move_to(xa,ya)
      self.ctx.line_to(xb,yb)
      self.ctx.stroke()

    for k in xrange(NUM):

      if self.F[k].sum() >= MAXFS:
        self.ctx.set_source_rgb(*CONTRASTB)
      else:
        self.ctx.set_source_rgb(*CONTRASTA)

      self.ctx.arc(self.X[k],self.Y[k],ONE*6,0,2*pi)
      self.ctx.fill()
